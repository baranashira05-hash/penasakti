...
import {
  Bold, Italic, Underline, Strikethrough, Quote, Code, Link2,
  Image as ImageIcon, Table as TableIcon,
  List, ListOrdered, Heading1, Heading2, Heading3,
  Undo, Redo, Eye, Save, Send,
  Tag, Globe, ChevronDown, Sparkles, X
} from "lucide-react";
...
export default function ArticleEditor({ mode, initialData }: ArticleEditorProps) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: "ProseMirror min-h-[500px] focus:outline-none",
      },
    },
    onUpdate: ({ editor }) => {
      // Auto save
  ...
  const handleSave = async (saveStatus = "DRAFT") => {
    ...
    try {
      ...
      const payload = {
        title, excerpt, content, categoryId: category,
        tags, featuredImage, metaTitle, metaDesc, metaKeywords,
        status: saveStatus,
      };
    ...
    <div className="flex gap-6">
      ...
      <div className="flex-1 min-w-0 space-y-4">
        ...
        <div>
          <input
            ...
            className="w-full text-3xl font-black border-0 border-b-2 border-border pb-3 bg-transparent focus:outline-none focus:border-penasakti-blue placeholder:text-muted-foreground/40 transition-colors"
          />
        ...
        <textarea
          ...
          placeholder="Ringkasan artikel (akan tampil di halaman utama dan hasil pencarian)..."
          rows={2}
          className="w-full px-4 py-3 rounded-xl border border-border bg-muted/30 focus:outline-none focus:ring-2 focus:ring-penasakti-blue/20 text-sm resize-none"
        />
            ...
            {editor && (
              <div className="border-b border-border p-2 flex flex-wrap gap-1 bg-muted/30">
                {[
                  ...
                  { icon: Strikethrough, action: () => editor.chain().focus().toggleStrike().run(), title: "Strikethrough", active: editor.isActive("strike") },
                  { icon: Code, action: () => editor.chain().focus().toggleCode().run(), title: "Code", active: editor.isActive("code") },
                  { type: "sep" },
            ...
            <div className="bg-card rounded-2xl border border-border p-5 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button
                    onClick={() => generateAI("meta-title")}
                    disabled={aiLoading === "meta-title"}
                ...
                />
                <p className={cn("text-xs mt-1", metaTitle.length > 60 ? "text-destructive" : "text-muted-foreground")}>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button
                    onClick={() => generateAI("meta-desc")}
                    disabled={aiLoading === "meta-desc"}
                    className="text-xs flex items-center gap-1 text-penasakti-blue hover:underline"
                  >
                    <Sparkles className="w-3 h-3" />
              ...
              {(metaTitle || title) && (
                <div className="p-4 bg-muted/30 rounded-xl">
                  ...
                  <p className="text-sm text-muted-foreground line-clamp-2 mt-1">
                    {metaDesc || excerpt || "Deskripsi artikel akan muncul di sini..."}
                  </p>
                </div>
              )}
            </div>
        ...
        {activeTab === "settings" && (
          <div className="bg-card rounded-2xl border border-border p-5 space-y-4">
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Contoh: Antara, Reuters"
                className="w-full px-4 py-2.5 rounded-xl border border-border bg-background focus:outline-none text-sm"
              />
            </div>
          </div>
      ...
      <div className="w-72 flex-shrink-0 space-y-4">
        ...
        <div className="bg-card rounded-2xl border border-border p-4 space-y-3">
          <h3 className="font-bold">Terbitkan</h3>
          <div className="grid grid-cols-2 gap-2">
            <button
              onClick={() => handleSave("DRAFT")}
              disabled={saving}
              className="flex items-center justify-center gap-2 py-2.5 border border-border rounded-xl text-sm font-medium hover:bg-muted transition-colors disabled:opacity-70"
            >
            ...
            <button
              onClick={() => {}}
              className="flex items-center justify-center gap-2 py-2.5 border border-border rounded-xl text-sm font-medium hover:bg-muted transition-colors"
            >
        ...
        <div className="bg-card rounded-2xl border border-border p-4">
          <label className="text-sm font-bold block mb-2">Kategori *</label>
          <select
            value={category}
